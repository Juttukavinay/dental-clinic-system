...
import { Sun, Moon, Clock, LogIn, LogOut, CheckCircle2, Menu } from 'lucide-react';
...
const Navbar = ({ onMenuToggle }) => {
  ...
  return (
    <header className="fixed right-0 top-0 z-10 flex h-16 left-0 lg:left-64 items-center justify-between border-b border-slate-200 bg-white/60 px-4 sm:px-6 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/60">
      {/* Menu Trigger & Greetings */}
      <div className="flex items-center gap-3">
        {/* Hamburger button on mobile */}
        <button
          onClick={onMenuToggle}
          className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 text-slate-600 transition-colors hover:bg-slate-100 lg:hidden dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800"
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`rounded-full px-2 py-0.5 text-[10px] sm:text-xs font-semibold uppercase tracking-wider ${getRoleBadgeColor(user?.role)}`}>
            {user?.role}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        {msg && (
          <span className="hidden sm:flex items-center gap-1.5 text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 px-3 py-1 rounded-lg">
        ...
        <div className="flex items-center gap-2 border-r border-slate-200 pr-4 dark:border-slate-800">
          {!isCheckedIn ? (
            ...
            >
              <LogIn size={13} />
              <span><span className="hidden sm:inline">Staff </span>Check-in</span>
            </button>
          ) : (
            <button
            ...
            >
              <LogOut size={13} />
              <span><span className="hidden sm:inline">Staff </span>Check-out</span>
            </button>
          )}
        </div>
